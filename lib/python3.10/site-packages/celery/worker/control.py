...
def conf(state, with_defaults=False, **kwargs):
    """List configuration."""
    return jsonify(main.app.conf.table(with_defaults=with_defaults),
...
def rate_limit(state, task_name, rate_limit, **kwargs):
    ...
    try:
        main.app.tasks[task_name].rate_limit = rate_limit
    except KeyError:
...
def clock(state, **kwargs):
    ...
    return {'clock': main.app.clock.value}
...
def pool_restart(state, modules=None, reload=False, reloader=None, **kwargs):
    ...
    if main.app.conf.worker_pool_restarts:
